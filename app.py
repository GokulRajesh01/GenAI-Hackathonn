...
def generate_pdf_template(form_data: dict) -> pathlib.Path:
    template_path = STATE_DIR / f"template_{form_data['form_id']}.pdf"
...
def fill_pdf(template_path: pathlib.Path, data: dict, form_id: str, session_id: str) -> str:
    out_path = OUTPUTS_DIR / f"{form_id}_{session_id}_filled.pdf"
...
def lambda_handler(event, context):
    try:
        ...
        api_key = get_api_key()
        if not api_key:
            return _respond(500, {"error": "GEMINI_API_KEY environment variable is missing"})
        ...
        if state["status"] == "INIT":
            ...
            # Generate the template asynchronously basically, before we need it
            generate_pdf_template(best_form)
        ...
        # Step 2: Collecting answers
            ...
            if state["current_field_index"] < len(state["field_keys"]):
                ...
                template_path = generate_pdf_template(state["form_data"])
                out_pdf = fill_pdf(template_path, cleaned_answers, state["form_data"]["form_id"], session_id)
